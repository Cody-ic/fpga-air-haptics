...
| 电脑软件 | 已实现任意位置放点、Demo、双向串口、普通／调试界面、状态与相位回读 | FPGA 固件配套、真实板卡联调与测量接入 |
...
- [上位机使用说明](desktop_app/README.md)：自由放点、普通／调试界面、Demo 与数据边界。
- [HAP2 串口协议](desktop_app/PROTOCOL.md)：坐标路径、设备能力声明、完整配置提交、原子状态快照和看门狗。
...
上位机默认打开画布，支持任意位置放点、拖动、精确坐标输入、缩放、保存与载入，最多 64 个节点。无需选择绘图网格；图形按实际坐标保存，换阵列不会改变图形位置。Demo 可设置 4×4、6×6、8×8、12×12、16×16 及自定义行列；真实硬件规模由 FPGA 握手声明。普通界面保留绘图与播放，勾选“调试模式”查看相位、声场和串口日志。
