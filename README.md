...
> 更新于 **2026-09-25**。已有电脑上位机、Windows 便携 EXE、Android 测试应用、Demo 模拟设备及双向通信适配。设备协议仍是上位机侧草案，待与 FPGA 端共同确定；蓝牙模块未选定。硬件仍处于方案设计与选型阶段，尚无 RTL、PCB 工程或硬件实测结果；软件演示不代表触觉效果已验证。
...
手机端使用 Flutter，进入 `mobile_app/` 后运行 `flutter pub get`、`flutter test`、`flutter run`。Windows 可在仓库根目录运行 `powershell -File mobile_app/build_android.ps1` 构建内部测试 APK，产物位于 `mobile_app/dist/`。手机向设备提交完整图形并接收状态；高速扫描仍由板端负责。详细环境及中文路径处理见手机应用说明。
