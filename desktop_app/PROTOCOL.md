...
Demo 运行时每 0.05 秒回传完整快照，其他状态每 0.5 秒回传，另外在控制命令后即时回传。Demo 使用内存传输，其运行回传量不作为串口带宽目标；真实固件应按波特率与帧长度安排回传。客户端每 0.8 秒发送 PING，ACK 超过 2 秒或状态超过 2.5 秒未到达则关闭会话；界面超过 1.6 秒标为过期，控制命令在 3.5 秒内没有后续匹配版本快照也会断开。
