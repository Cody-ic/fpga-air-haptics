...
`desktop_app/` contains the Python application: `app.py` handles Tk UI, `editor.py` edits coordinate paths, `model.py` supplies the reference model, and `protocol.py`, `controller.py`, `transport.py`, and `demo.py` handle communication. Tests live in `desktop_app/tests/`. Wire semantics belong in `desktop_app/PROTOCOL.md`. No HDL, PCB, or hardware measurements exist yet.
...
Tang Mega 60K is selected; hardware starts at 4×4 then expands to 8×8. Drawings use arbitrary coordinates, never emitter-grid indices. Current software supports 64 ordered vertices and a single moving focus. FPGA owns eventual high-speed scanning; Demo is a slow path demonstration. Preserve all coordinates when switching arrays. Distinguish simulated states, digital readback, theory, and measurements; prioritize single-focus validation before extensions.
